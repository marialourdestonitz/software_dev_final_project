...
def add_student():
    if idEntry.get()=='' or firstNameEntry.get()=='' or lastNameEntry.get()=='' or genderBox.get()=='' or mobileEntry.get()=='' or courseBox.get()=='':
        messagebox.showerror('Error','Please fill all fields')
    elif smsdb.student_id_exists(idEntry.get()):
        messagebox.showerror('Error', 'Student ID already exists')
    else:
        smsdb.insert(idEntry.get(), firstNameEntry.get(), lastNameEntry.get(), genderBox.get(), mobileEntry.get(), courseBox.get())
        mainview_data()
        clear()
...
def update_student():
    ...
    if not selected_item:
        ...
        smsdb.update_students(idEntry.get(), firstNameEntry.get(), lastNameEntry.get(), genderBox.get(), mobileEntry.get(), courseBox.get())
        mainview_data()
        clear()
...
def delete_student():
    ...
    if not selected_item:
        ...
        smsdb.delete_student(idEntry.get())
        mainview_data()
        clear()
...
def search_student():
    if searchEntry.get()=='':
        ...
        searched_data = smsdb.search(searchBox.get(), searchEntry.get())
        tree.delete(*tree.get_children())
...
#Add ID Label
idLabel = CTkLabel(leftFrame, text='Student ID:', font=('Arial', 18, 'bold'))
...
courseLabel.grid(row=5, column=0,padx=20,pady=15,sticky=W)
...
courseBox.grid(row=5, column=1,padx=20,pady=15,sticky=W)
...
me = CTkFrame(student_window,fg_color='#00827f')
